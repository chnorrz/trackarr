...
// Optional upstream proxy for providers that ask for it (opts.proxy).
...
// tools/ipv6-proxy.js - 1337x bans our IPv4 but not our IPv6, and the
// Colima VM has no IPv6 of its own). Playwright's Firefox wants the proxy
// set at launch, so this is a second browser rather than a second context.
...
// browser/context).
//
// opts.proxy routes the request through PROXY_URL instead of the default
// (direct) browser.
export async function gotoCleared(url, opts = {}) {
  const { timeoutMs = 30000, proxy = false } = typeof opts === 'number' ? { timeoutMs: opts } : opts;
  console.error(`[cf] gotoCleared: ${url}${proxy ? ' (via proxy)' : ''}`);

  const context = (proxy ? await getProxyContext() : null) || await getPersistentContext();
  const usingProxy = proxy && context === proxyContext;
  if (proxy && !usingProxy) {
    console.error('[cf] PROXY_URL not set, falling back to a direct connection.');
  }
