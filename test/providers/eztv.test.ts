...
test('eztv search() throws when every row is filtered out, instead of returning an empty result', async () => {
  ...
  cfFetch.mock.mockImplementation(async () => toRes(allLinklessHtml));
...
test('eztv resolveMagnet() serves from magnetCache after a prior search(), no second fetch', async () => {
  ...
  const resolved = await provider.resolveMagnet({ id: null, url: detailUrl });

  assert.equal(resolved.kind, 'magnet');
  assert.ok(resolved.magnet.startsWith('magnet:?xt=urn:btih:0000000000000000000000000000000000aaa1'));
...
test('eztv resolveMagnet() falls back to a detail-page fetch on a cache miss', async () => {
  ...
  cfFetch.mock.mockImplementation(async () => toRes(detailHtml));

  const resolved = await provider.resolveMagnet({ id: null, url: 'https://eztvx.to/ep/9999999/never-searched/' });
  assert.equal(resolved.kind, 'magnet');
  assert.ok(resolved.magnet.startsWith('magnet:?xt=urn:btih:0000000000000000000000000000000000eee1'));
});
